...
create table RATING (
	...
	constraint RATING_KEY primary key (PLAYER_ID, SET_DATE)
);
...
alter table RATING add
     constraint FK_RATING_P        foreign key (PLAYER_ID)    
	 references PLAYER (PLAYER_ID) on update no action on delete no action
